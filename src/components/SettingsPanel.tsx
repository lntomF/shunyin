...
export function SettingsPanel({ dict, open, onClose }: SettingsPanelProps) {
  ...
  return (
    <div className="fixed inset-0 z-[65] flex items-center justify-center overflow-y-auto bg-surface-container-lowest/76 px-3 py-4 backdrop-blur-2xl sm:px-4">
      <div className="studio-panel relative max-h-[calc(100dvh-2rem)] w-full max-w-2xl overflow-y-auto rounded-2xl shadow-[0_30px_80px_rgba(0,0,0,0.28)] sm:rounded-3xl">
        ...
        <div className="relative flex items-start justify-between gap-4 border-b border-outline-variant/10 px-6 py-5 md:px-7">
          <div className="max-w-xl">
            ...
            <h2 className="mt-3 font-headline text-2xl font-bold tracking-[0] text-primary md:text-[2rem]">{dict.settingsTitle}</h2>
          ...
          <button
            ...
            className="dock-shell flex h-10 w-10 items-center justify-center rounded-xl text-outline shutter-transition hover:-translate-y-0.5 hover:text-primary"
          >
            <X size={16} />
          </button>
        </div>
        ...
        <div className="relative space-y-5 px-6 py-6 md:px-7 md:py-7">
          <section className="studio-panel relative overflow-hidden rounded-2xl p-5 md:p-6">
            <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-secondary/60 to-transparent" />
            <div className="flex items-start gap-4">
              <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border border-secondary/15 bg-surface-container-high/85 text-secondary">
                <ShieldCheck size={18} />
              </div>
              <div className="min-w-0">
                <div className="text-[10px] font-bold uppercase tracking-[0.24em] text-secondary">{dict.settingsExperimentalLabel}</div>
                <h3 className="mt-2 font-headline text-xl font-bold tracking-[0] text-primary">{dict.settingsLocalSectionTitle}</h3>
          ...
          <section className="grid gap-3 sm:grid-cols-2">
            {workflowItems.map(({ icon: Icon, text }) => (
              <div key={text} className="hover-lift rounded-2xl border border-secondary/10 bg-surface/65 p-4 shutter-transition">
                <div className="mb-3 flex h-9 w-9 items-center justify-center rounded-xl border border-secondary/15 bg-surface-container-high/80 text-secondary">
                  <Icon size={15} />
                </div>
