...
export function MinimalWhiteFooterTemplate({
  ...
  const footerHeight = Math.max(
    height - sourceHeight,
    clamp(Math.round(isPortrait ? sourceWidth * 0.18 : sourceHeight * 0.135), isPortrait ? 112 : 88, scaledMax(isPortrait ? 220 : 158)),
  );
  ...
  const labelSize = isPortrait
    ? clamp(Math.round(sourceHeight * 0.01), 16, scaledMax(42))
    : clamp(Math.round(width * 0.0078), 8, scaledMax(12));
  const cameraSize = isPortrait
    ? clamp(Math.round(sourceHeight * 0.022), 30, scaledMax(92))
    : clamp(Math.round(width * 0.017), 15, scaledMax(27));
  const detailSize = isPortrait
    ? clamp(Math.round(sourceHeight * 0.016), 24, scaledMax(68))
    : clamp(Math.round(width * 0.0115), 11, scaledMax(18));
  const smallSize = isPortrait
    ? clamp(Math.round(sourceHeight * 0.011), 16, scaledMax(46))
    : clamp(Math.round(width * 0.009), 9, scaledMax(13));
  ...
  const lensText = lensModel ? truncateText(lensModel, maxChars(centerWidth * 0.48, detailSize)) : null;
  ...
  const portraitCameraText = truncateText(cameraTitle, maxChars(width * 0.78, cameraSize));
  const portraitInfoText = truncateText(lensModel ? `${lensModel} · ${parameterLine}` : parameterLine, maxChars(width * 0.82, detailSize));
    ...
    <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {isPortrait ? (
        ...
        <>
          ...
          {lensText && <circle cx={centerX} cy={mainY - detailSize * 0.34} r={Math.max(2, detailSize * 0.13)} fill="#C8BFAF" />}
          <text
            x={lensText ? centerX + splitGap : centerX}
            y={mainY}
            fill="#121212"
            fontSize={detailSize}
            fontFamily="Inter, Arial, sans-serif"
